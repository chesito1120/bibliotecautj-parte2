...
class MaestroController extends Controller
{
    ...
    public function uploadCSV(Request $request)
    {
        ...
        try {
            // Importar el archivo CSV usando un import personalizado para Maestro
    ...
    public function store(Request $request)
    {
        ...
        // Asignar 'tipo_usuario' con valor 'Docente'
        $data = $request->all();
        $data['tipo_usuario'] = 'Docente'; // Agregar tipo_usuario con valor 'Docente'

        // Crear un nuevo maestro con los datos validados y tipo_usuario
        Maestro::create($data);
    ...
    {
        // Validar la solicitud con condición en la validación del número de empleado
        $request->validate([
            'numero_empleado' => [
                'required',
                'integer',
                function ($attribute, $value, $fail) use ($maestro) {
                    // Solo validar si el número de empleado ha cambiado
                    if ($value !== $maestro->numero_empleado && Maestro::where('numero_empleado', $value)->exists()) {
                        $fail('El número de empleado ya está en uso.');
                    }
                },
            ],
            'nombre' => 'required|string',
        ...
        if ($handle) {
            ...
            while (($row = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                $maestroData = [
                    ...
                    'sexo' => $row[4],
                    'puesto' => $row[5],
                    'tipo_usuario' => 'Docente', // Asignar 'Docente' al campo tipo_usuario
                ];
